...
ARCHIVES_ROOT = ET.parse(
    os.path.join(ARCHIVES_DIR, 'expected_results.xml')).getroot()
...
def setup_config(scope='module'):
    "Setup logging based on yakunin.json (if present)"
    ...
    if not os.path.exists(config_file):
        config_file = os.path.join(
            os.path.dirname(
                os.path.realpath(yakunin.__file__)),
            '..', '..', '..', '..', 'etc', 'yakunin.json')
...
def archive_compilation_tester(mount_wjfs):
    """General fixture to test the compilation of a given archive.
    The "xml_desc" is an ElementTree.Element
    (see also tests/test-files/expected_results.xml)
    which contains the archive name to compile and a list
    of lines that must be present in the compilatin log."""
    ...
    def archive_compilation_tester(xml_desc):
        ...
        if archive_type is not None:
            if archive_type == 'preprint':
                ...
                assert preprint is not None, f"Expecting a preprint number for {filename}"
                filename = os.path.join(
                    mount_wjfs, preprint, '1', 'submission', filename)
            else:
                assert False, f"Unknown archive type {archive_type}"
        else:
        ...
        if options is not None:
            ...
            options = dict(timeout=300)  # ← why???

        # print(options)
        # print(command)

        # result = getattr(Archive(archive=filename), command)(options)
        ...
        with well_formed(result) as data:
            ...
            with open(os.path.join(tmp_dir, TASK_LOG), 'r') as src:
                ...
                for expected_line in expected_lines:
                    ...
                    if expected_text.find("💩") > -1:
                        ...
                        found = list(
                            filter(lambda x: re.match(pattern, x), log_lines))
            ...
            for expected_file in xml_desc.iter('file'):
                assert os.path.exists(
                    os.path.join(tmp_dir, 'work', expected_file.text))
...
@pytest.fixture(params=[['bertieri', 'jhep', None], ])
def mount_wjfs(request, scope='session'):
    """Mount JOURNAL@SERVER:JOURNALArchive/archive/papers/preprint
    into /tmp/JOURNAL (or dst if provided).
    List all preprints therein and find the submitted archive for each.
    Yield a list of such archives.
    Unmount at tear-down"""
    ...
    # see note above...
    if os.listdir(dst):
        subprocess.run(args=['fusermount',
                             '-u',
                             dst],
                       check=True)
...
def well_formed(result_archive):
    """Check that the given tar.gz (str or path) contains dirs
    "submission", "work" and the task log. Yield a tuple containing
    the tempdir where the tar.gz has been extracted and the list of
    files therein.
    """
...
def pytest_generate_tests(metafunc):
    """Use command-line option as parameter of test functions
    that require a fixture named "preprint" """
    if "preprint" in metafunc.fixturenames:
        metafunc.parametrize(
            "preprint", metafunc.config.getoption("prepare_preprint"))
