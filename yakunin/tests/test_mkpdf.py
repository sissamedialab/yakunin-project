...
def test_simple_pdf(setup_config):
    ...
    pdf_file_path = ARCHIVES_DIR / pdf_file
    ...
    with well_formed(result) as (tmp_dir, files):
        ...
        task_log = tmp_dir / TASK_LOGFILE_NAME
        log_lines = task_log.read_text(encoding="utf-8").split("\n")

        # the log reports that yakunin simply copied the given pdf file
        found = list(
            filter(
                lambda x: re.match(
                    r"INFO Unpacked .+submission/14-test.pdf as copy",
                    x,
                ),
                log_lines,
            ),
        ...
        assert "INFO Main pdf is 14-test.pdf" in log_lines
...
def test_from_tex_archive(setup_config):
    ...
    archive_file_path = ARCHIVES_DIR / archive_file
...
def test_from_odt(filename, mime, setup_config):
    ...
    archive_file_path = ARCHIVES_DIR / filename
    archive = Archive(archive=archive_file_path)
    archive.mkpdf()
    ...
    with well_formed(result) as (tmp_dir, files):
        ...
        logfile_path = tmp_dir / TASK_LOGFILE_NAME
        log = logfile_path.read_text(encoding="utf-8")
