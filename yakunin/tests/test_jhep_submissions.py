...
def test_collect_preprint(mount_wjfs, preprint, setup_config):
    ...
    filename = aruspica_submitted_archive(pathlib.Path.cwd())
    result = Archive(archive=filename).tex_compile(timeout=300)
    print(result)
    assert 1 == 0  # noqa: PLR0133


def aruspica_submitted_archive(basedir) -> str:  # noqa: PLR0911
    """
    ...
    if len(arxivs) > 1:
        YAKUNIN_LOGGER.debug("Troppi file che sembrano arXivs... {arxivs}")
    ...
    for extension in ["tar.gz", "zip", "tex.gz", "tex"]:
        ...
        if len(candidates) > 1:
            raise ValueError(f"Troppi file {extension}... " + str(candidates))
