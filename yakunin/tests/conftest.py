...
ARCHIVES_DIR = pathlib.Path(os.path.realpath(__file__)).parent / "test-files"

ARCHIVES_ROOT = ET.parse(ARCHIVES_DIR / "expected_results.xml").getroot()
...
def setup_config(scope="module"):
    """Configure logging based on yakunin.json."""
    # TODO: refactor with utils.setup_yakunin
    config_file = files("yakunin") / "yakunin.json"
    args = argparse.Namespace()
    if not config_file.exists():
        pytest.fail(f"No config file {config_file} found")
    args.config_file = config_file
    merge_with_config_file(args)
    return args


def _archive_compilation_tester(xml_desc):  # noqa: PLR0914
    ...
    command = command.text if command is not None else "tex_compile"
...
def archive_compilation_tester(mount_wjfs):
    ...
    """
    return _archive_compilation_tester
...
def mount_wjfs(request, scope="session"):
    ...
    Yields:
      str: the path of the mounted dir.  Unmount at tear-down.
...
@contextmanager
def well_formed(result_archive) -> (Path, str):
    """
    ...
    Yields:
      tuple: the tempdir where the tar.gz has been extracted
             and the list of files therein.
    ...
    try:
        ...
        assert TASK_LOGFILE_NAME in files, "Missing log file"
